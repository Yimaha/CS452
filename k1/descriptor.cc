 ...
 * initialization,
 */
TaskDescriptor::TaskDescriptor(int id, int parent_id, int priority, void (*pc)()) : task_id{id}, parent_id{parent_id}, priority{priority}, prepared_response{0x0}, alive{true},
                                                                                    initialized{false}, pc{pc}
{
    sp = (char *)&kernel_stack[4096]; // aligned to 8 bytes, exactly 4kb of data is used
}
